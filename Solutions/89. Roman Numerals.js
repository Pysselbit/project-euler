...
for (r of ROMAN_NUMERALS)
	saved += r.length - decimalToRoman(romanToDecimal(r)).length;
...
function romanToDecimal(roman) {
	...
	for (i = 0; i < roman.length; i++) {
		var value = ROMAN_DIGITS[roman[i]];

		if (i + 1 < roman.length) {
			if (value < ROMAN_DIGITS[roman[i + 1]])
				value *= -1;
		}
...
function decimalToRoman(decimal) {
	...
	for (var i = Object.keys(ROMAN_DIGITS).length - 1; i >= 0; i--) {
		var romanValue = Object.keys(ROMAN_DIGITS)[i];
		var decimalValue = Object.values(ROMAN_DIGITS)[i];

		var n = Math.floor(decimal / decimalValue);

		roman += romanValue.repeat(n);
		decimal -= n * decimalValue;

		if (i > 0) {
			for (var subtractor in ROMAN_SUBTRACTIONS) {
				if (ROMAN_SUBTRACTIONS[subtractor].includes(romanValue)) {
					var romanSubtractor = subtractor;
					var decimalSubtractor = ROMAN_DIGITS[romanSubtractor];

					if (decimal >= decimalValue - decimalSubtractor) {
						roman += romanSubtractor + romanValue;
						decimal -= decimalValue - decimalSubtractor;
					}
